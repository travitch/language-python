...
runQ q = evalState tM rwState
  where
    ...
    rewriteIds (UniqueIdent base uid annot) = do
      s <- get
      case M.lookup uid (nameMap s) of
        Just assignedName -> return assignedName
        Nothing -> assignUniqueName base uid annot
    -- This is a capturing identifier and does not need to be
    -- rewritten
    rewriteIds x = return x
...
tryAS annot body excepts els finally = do
  body' <- sequence body
